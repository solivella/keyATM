...
check_multi <- function(docs){
  if(!is.list(docs) | !all(sapply(docs, function(x)"keyATM_docs" %in% class(x)))){
    stop("When model is 'multicorpora', docs should be a list with each element generated by keyATM_read().")
  }
}
...
full_model_name <- function(model = c("base", "covariates", "dynamic", "label", "multicorpora"),
                            type = c("keyATM", "lda"))
{
